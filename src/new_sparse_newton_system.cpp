...
void F(
    ...
    //////////////////////////////////////////////////////////////
    // define residual y(i) here using cmath expressions        //
    // refere to your $x_{i}$ and $p_{i}$ as x[i] / p[i]        //  
    //////////////////////////////////////////////////////////////

    for(size_t i = 1; i < N-1; i++){
        y(i) = x(i-1)-(p(0)-p(1)*x(i))*x(i)+p(2)*x(i+1)-p(3);
    }
    y(0) = -(p(0)-p(1)*x(0))*x(0)+p(2)*x(1)-p(3);
    y(N-1) = x(N-2)-(p(0)-p(1)*x(N-1))*x(N-1)-p(3);
}
